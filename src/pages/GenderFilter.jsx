...
const GenderFilter = () => {
    ...
    const filteredProducts = products && Array.isArray(products)
        ? products.filter(product => product.gender.toLowerCase() === category.toLowerCase())
        : [];

    const nextProduct = currentPage * itemsPerPage
    const firstPage = nextProduct - itemsPerPage
    const currentProducts = filteredProducts.slice(firstPage, nextProduct)
                ...
                <div className={`pt-7 ${
                    ...
                    {currentProducts.length > 0 && (
                        <div className="flex justify-between items-center">
                            <h2 className="htwo ml-10 lg:ml-4">{ category }</h2>
                            <img 
                                src={filter} 
                                alt="Filter Icon"
